...
export class MCPClient {
  ...
  async connect(): Promise<void> {
    ...
    try {
      ...
      if (this.serverParams.allowedDirectory) {
        ...
        logger.debug(
          `[MCP Client] Using working directory: ${spawnOptions.cwd}`
        );
      }
      ...
      if (this.serverParams.env) {
        ...
        logger.debug(
          `[MCP Client] Environment variables set: ${Object.keys(
            this.serverParams.env
          ).join(", ")}`
        );
      }
      ...
      logger.debug(
        `[MCP Client] Spawning process: ${
          this.serverParams.command
        } ${this.serverParams.args?.join(" ")}`
      );
      ...
      if (this.stdout) {
        this.stdout.on("data", (data: Buffer) => {
          logger.debug(
            `[MCP Client] Received raw data: ${data.toString().trim()}`
          );
      ...
      logger.error(
        `[MCP Client] Connection failed: ${error?.message || String(error)}`
      );
      throw error;
    }
  }
  ...
  private async initialize(): Promise<void> {
    ...
    try {
      ...
      if (!response || typeof response.protocolVersion !== "string") {
        throw new Error(
          "[MCP Client] Invalid initialization response from server"
        );
      }
      ...
      logger.debug(
        `[MCP Client] Server version: ${JSON.stringify(this.serverVersion)}`
      );
      logger.debug(
        `[MCP Client] Server capabilities: ${JSON.stringify(
          this.serverCapabilities
        )}`
      );
    } catch (error: any) {
      logger.error(
        `[MCP Client] Session initialization failed: ${
  ...
  private async updateAvailableTools(): Promise<void> {
    try {
      ...
      logger.debug(
        `[MCP Client] Updated available tools: ${Array.from(
          this.availableTools
        ).join(", ")}`
      );
    } catch (error) {
  ...
  private handleResponse(data: Buffer) {
    ...
    for (const message of messages) {
      try {
        ...
        logger.debug(
          `[MCP Client] Parsed message: ${JSON.stringify(response)}`
        );
        ...
        if (pendingMessage) {
          if (response.error) {
            logger.error(
              `[MCP Client] Message error: ${response.error.message}`
            );
            pendingMessage.reject(new Error(response.error.message));
          } else {
            logger.debug(
              `[MCP Client] Message success: ${JSON.stringify(response.result)}`
            );
  ...
  async getAvailableTools(): Promise<any[]> {
    ...
    try {
      ...
      logger.error(
        `[MCP Client] Failed to get tools: ${error?.message || String(error)}`
      );
      throw error;
    }
  }
  ...
  async callTool(toolName: string, toolArgs: any): Promise<any> {
    ...
    try {
      ...
      const response = await this.sendMessage(message);
      logger.debug(
        `[MCP Client] Tool call response: ${JSON.stringify(response)}`
      );
      return response;
    } catch (error: any) {
      logger.error(
        `[MCP Client] Tool call failed: ${error?.message || String(error)}`
      );
      throw error;
    }
  }
